...
from llm_func import grade_essay_test, similar_grade,grade_essay_origin
...
def process_essay(input_num, root_dir, output_dir):
    ...
    try:
        ...
        # 4. 写入KNN结果（使用专用锁）
        knn_output_path = rf"D:\zuowen\output\KNNS\knn{input_num}.txt"

        with knn_file_lock:
            with open(knn_output_path, 'w', encoding='utf-8') as output_knn:
                ...
                for i in docs:
                    ...
                    if count == 1 or i['_source']['id'] == input_num:
                        ...
                        continue
                    if count>2 and similar_grade(user_input, str(i['_source']['title']) + '\n' + str(i['_source']['text']), DASHSCOPE_API_KEY).startswith('无关'):
                        continue
                    tcnt+=1
                    if tcnt>5:
                        break
        ...
        # 5. 读取KNN结果（使用专用锁）
        with knn_file_lock:
            with open(knn_output_path, 'r', encoding='utf-8') as file:
                samples = file.read().rstrip()

        
        # 6. 模型批改（无文件操作，不需要锁）
        print(f"作文 {input_num} - 正在使用deepseek-v3.1模型批改作文，请稍候...(RAG)\n")
        result_none = grade_essay_origin(title, content, samples, DASHSCOPE_API_KEY, None, None,"deepseek-v3.2")
...
if __name__ == "__main__":
    ...
    output_dir = r"D:\zuowen\data\eval\temp\origin"  # 输出目录
