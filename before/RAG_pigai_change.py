...
from llm_func import grade_essay_test, similar_grade_content,similar_grade_structure,similar_grade_language,join_result,grade_essay_origin
...
def process_essay(input_num, root_dir, output_dir):
    ...
    try:
        ...
        result1, result2, result3 = split_list_to_three_balanced_randomly(docs[4:])
        result1 = docs[0:3]+result1
        result2 = docs[0:3]+result2
        result3 = docs[0:3]+result3
        
        # 4. 写入KNN结果（使用专用锁）
        knn_output_path_1 = rf"D:\zuowen\output\KNNS\knn_1_{input_num}.txt"
        knn_output_path_2 = rf"D:\zuowen\output\KNNS\knn_2_{input_num}.txt"
        knn_output_path_3 = rf"D:\zuowen\output\KNNS\knn_3_{input_num}.txt"
        with knn_file_lock:
            with open(knn_output_path_1, 'w', encoding='utf-8') as output_knn:
                ...
                for i in result1:
                    ...
                    if count == 1 or i['_source']['id'] == input_num:
                        ...
                        continue
                    if count>2 and similar_grade_content(user_input, str(i['_source']['title']) + '\n' + str(i['_source']['text']), DASHSCOPE_API_KEY).startswith('无关'):
                        continue
                    tcnt+=1
                    if tcnt>10:
                        break
                    output_knn.write(str(i['_source']['title']) + '\n' + str(i['_source']['text']) + '\n' + str(i['_source']['comment']))
                    output_knn.write("----------------------------" * 20 + "\n")
                    print(f"作文 {input_num} - {i['_source']['id']}    {i['_score']}")
            with open(knn_output_path_2, 'w', encoding='utf-8') as output_knn:
                print(f"作文 {input_num} - ---------------------------------------")
                count = 0
                tcnt=0
                for i in result2:
                    ...
                    if count == 1 or i['_source']['id'] == input_num:
                        print(f"作文 {input_num} - {i['_source']['id']}    {i['_score']}")
                        continue
                    if count>2 and similar_grade_structure(user_input, str(i['_source']['title']) + '\n' + str(i['_source']['text']), DASHSCOPE_API_KEY).startswith('无关'):
                        continue
                    tcnt+=1
                    if tcnt>10:
                        break
                    ...
                    output_knn.write("----------------------------" * 20 + "\n")
                    print(f"作文 {input_num} - {i['_source']['id']}    {i['_score']}")
            with open(knn_output_path_3, 'w', encoding='utf-8') as output_knn:
                print(f"作文 {input_num} - ---------------------------------------")
                count = 0
                tcnt=0
                for i in result3:
                    ...
                    if count == 1 or i['_source']['id'] == input_num:
                        print(f"作文 {input_num} - {i['_source']['id']}    {i['_score']}")
                        continue
                    if count>2 and similar_grade_language(user_input, str(i['_source']['title']) + '\n' + str(i['_source']['text']), DASHSCOPE_API_KEY).startswith('无关'):
                        continue
                    tcnt+=1
                    if tcnt>10:
                        break
        ...
        with knn_file_lock:
            ...
            with open(knn_output_path_2, 'r', encoding='utf-8') as file:
                second_samples = file.read().rstrip()
            with open(knn_output_path_3, 'r', encoding='utf-8') as file:
                third_samples = file.read().rstrip()
        ...
        result_1 = grade_essay_origin(title, content, first_samples, DASHSCOPE_API_KEY, rule, None,"deepseek-v3.2")
        result_2 = grade_essay_origin(title, content, second_samples, DASHSCOPE_API_KEY, rule, None,"deepseek-v3.2")
        result_3 = grade_essay_origin(title, content, third_samples, DASHSCOPE_API_KEY, rule, None,"deepseek-v3.2")
        result_none = join_result(result_1, result_2, result_3,content,DASHSCOPE_API_KEY,rule)
...
if __name__ == "__main__":
    # 修改路径配置（移除 file_path）
    root_dir = r'D:\zuowen\data\test'
    output_dir = r"D:\zuowen\data\eval\temp\change"  # 输出目录
