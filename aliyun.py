...
def process_essay(input_num, root_dir, output_dir):
    ...
    try:
        ...
        # 6. 模型批改（无文件操作，不需要锁）
        print(f"作文 {input_num} - 正在使用{LLM_MODEL}模型批改作文，请稍候...(RAG)\n")
        rule_path = r"D:\zuowen\data\eval\rule.txt"

        with open(rule_path, 'r', encoding='utf-8') as file:
            rule = file.read().rstrip()

        result_none = grade_essay_no_change(title, content, first_samples,DASHSCOPE_API_KEY,rule,None, LLM_MODEL)
...
if __name__ == "__main__":
    ...
    output_dir = rf"D:\zuowen\data\eval\temp\\"+LLM_MODEL  # 输出目录
